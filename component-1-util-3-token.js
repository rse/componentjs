...
_cs.token.prototype.setText = function (text) {
    this.text = text;
};

/*  setter for additional token symbols  */
_cs.token.prototype.addToken = function (b1, b2, e2, e1, symbol) {
    this.tokens.push({ b1: b1, b2: b2, e2: e2, e1: e1, symbol: symbol });
    this.len++;
};

/*  peek at the next next or token at particular offset  */
_cs.token.prototype.peek = function (offset) {
    if (typeof offset === "undefined")
        offset = 0;
    if (offset >= this.len)
        throw new Error("parse error: not enough tokens: " + this.ctx());
    return this.tokens[this.pos + offset].symbol;
};

/*  skip one or more tokens  */
_cs.token.prototype.skip = function (len) {
...
_cs.token.prototype.consume = function (symbol) {
    if (this.len <= 0)
...
_cs.token.prototype.ctx = function (width) {
